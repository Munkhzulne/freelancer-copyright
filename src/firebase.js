...
export const createRecordWithAccount = (path, data) => {
  ...
  return firebase.firestore().collection(`${ACCOUNT_HOME}/${uid}/${path}`).add({
...
export const updateRecordWithAccount = (path, data) => {
  let { uid } = firebase.auth().currentUser;
  data.createdAt = data.createdAt ? data.createdAt : firebase.firestore.FieldValue.serverTimestamp();
  return firebase.firestore().doc(`${ACCOUNT_HOME}/${uid}/${path}`).set(
      {
...
export const useCollectionWithAccount = (path) => {
  ...
  useEffect(
      () => {
          ...
          let subscription = collectionData(firestore.collection(`${ACCOUNT_HOME}/${uid}/${path}`).orderBy('createdAt', "desc"), 'id').subscribe((d) => {
              setData(d);
              setLoading(false);
          });
          return () => {
...
export const useCollection = (path) => {
  ...
  useEffect(
      () => {
          ...
          let subscription = collectionData(firestore.collection(`${path}`), 'id').subscribe((d) => {
              setData(d);
              setLoading(false);
          });
          return () => {
